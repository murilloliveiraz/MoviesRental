    ...
    public class ApiController : ControllerBase
    {
        protected ActionResult CustomResponse(int status, bool success, object data = null)
        {
            return (status, success) switch
            {
                ...
                (201, false) => NotFound(new BaseResponse { StatusCode = status, Success = success, Message = "Created.", Data = data }),
                (200, false) => NotFound(new BaseResponse { StatusCode = status, Success = success, Data = data })
            };
        }
    }
}
